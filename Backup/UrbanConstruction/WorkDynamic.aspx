<%@ Page Language="C#" MasterPageFile="~/WebSite.Master" AutoEventWireup="true" CodeBehind="WorkDynamic.aspx.cs" Inherits="UrbanConstruction.WorkDynamic" Title="工作动态" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
<script src="js/jquery-1.9.1.js" type="text/javascript"></script>
<script type="text/javascript">
    $(document).ready(function() {
        $("#mySelect").change(function() {
        window.location.href = "WorkDynamic.aspx?pageIndex=" + $("#mySelect").val() + "";
        });
    });
</script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
 <div class="middle"><!--S中间内容区域-->
    	<p class="dw">您现在的位置： <a href="index.aspx">首页</a>&nbsp;&nbsp;<span>&gt;&gt;</span>&nbsp;&nbsp;<%=menuname %></p>
        <div class="List"><!--S列表内容-->	
        	<div class="ListL">
            	 	<div class="LZCFG"><!--S工作动态-->
                	<h2><span>工作动态</span></h2>
                	<ul>
                	<%for (int i = 0; i < menulist.Count; i++)
                   { %>
                        <li>
                        <a href="WorkDynamic.aspx?kind=<%=menulist[i].Kind.ToString() %>">
                          <%=menulist[i].M_ItemName %>
                        </a>
                        </li>
                	<%} %>
        ...
        <ul>
            ...
            <div class="ListR">
               ...
               { %>
                <li>
                   <a href="ForExcel.aspx?searchtype=null&value=null&pageIndex=1&token=<%=Session["CRSFToken"] %>" target="_blank">
                        建筑工程档案预验收
                   </a>
                   <span><%=newsList[0].ReleaseTime.ToString("yyyy-MM-dd")%></span></li>
                    <li>
                   <a href="ForExcelGuanXIan.aspx?searchtype=null&value=null&token=<%=Session["CRSFToken"] %>" target="_blank">
                     管线工程档案预验收                
                   </a>
                   <span><%=newsList[0].ReleaseTime.ToString("yyyy-MM-dd")%></span></li>
                   ...
                   {%>
                   <li>
                   <a href="NewsDetail.aspx?FID=<%=newsList[i].ID.ToString()%>" target="_blank">
                     <%if (newsList[i].Title.Length > 32)
                              ...
                              <%else
                                { %>
                                 <%=newsList[i].Title.Substring(0, newsList[i].Title.Length - 1)%>
                                <%} %>
               ...
               <%else
                ...
                <%else
                    ...
                    <%if (kindid != "")
                      {%>
                       <a href="WorkDynamic.aspx?kind=<%=kindid %>&pageIndex=1">第一页</a>&nbsp;&nbsp;
                       <a href="WorkDynamic.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex-1 %>">上一页</a>&nbsp;&nbsp;
                      <%} %>
                      <%else
                        { %>
                         <a href="WorkDynamic.aspx?pageIndex=1">第一页</a>&nbsp;&nbsp;
                         <a href="WorkDynamic.aspx?pageIndex=<%=pageIndex-1 %>">上一页</a>&nbsp;&nbsp;
                        <%} %>
                ...
                <%else
                    ...
                    <%if (kindid != "")
                      {%>
                        <a href="WorkDynamic.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                        <a href="WorkDynamic.aspx?kind=<%=kindid %>&pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                      <%} %>
                      <%else
                        { %>
                          <a href="WorkDynamic.aspx?pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                          <a href="WorkDynamic.aspx?pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                      <%} %>
