<%@ Page Language="C#" MasterPageFile="~/WebSite.Master" AutoEventWireup="true" CodeBehind="Guide.aspx.cs"
    Inherits="UrbanConstruction.Guide" Title="办事指南" %>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $("#mySelect").change(function() {
                window.location.href = "Guide.aspx?pageIndex=" + $("#mySelect").val() + "";
            });
        });
    </script>
                ...
                <div class="LZCFG">
                    ...
                    <ul>
                        <%for (int i = 0; i < menulist.Count; i++)
                          { %>
                        <li><a href="Guide.aspx?kind=<%=menulist[i].Kind.ToString() %>">
                            <%=menulist[i].M_ItemName.ToString() %></a></li>
                        <%} %>
                    </ul>
                </div>
                <!--E工作动态-->
            ...
            <div class="ListR">
                <ul>
                    ...
                    <li><a href="NewsDetail.aspx?FID=<%=newsList[i].ID.ToString()%>" target="_blank">
                        ...
                        <%=newsList[i].ReleaseTime.ToString("yyyy-MM-dd")%></span></li>
                    <%} %>
                </ul>
                <p class="Page">
                    ...
                    <%if (kindid != "")
                      {%>
                    <a href="Guide.aspx?kind=<%=kindid %>&pageIndex=1">第一页</a>&nbsp;&nbsp; <a href="Guide.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex-1 %>">
                        上一页</a>&nbsp;&nbsp;
                    <%} %>
                    <%else
                        { %>
                    <a href="Guide.aspx?pageIndex=1">第一页</a>&nbsp;&nbsp; <a href="Guide.aspx?pageIndex=<%=pageIndex-1 %>">
                        上一页</a>&nbsp;&nbsp;
                    <%} %>
                    <%} %>
                    ...
                    <%if (kindid != "")
                      {%>
                    <a href="Guide.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                    <a href="Guide.aspx?kind=<%=kindid %>&pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                    <%} %>
                    <%else
                        { %>
                    <a href="Guide.aspx?pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp; <a href="Guide.aspx?pageIndex=<%=pageCount %>">
                        尾页</a>&nbsp;&nbsp;
                    <%} %>
                    <%} %>
