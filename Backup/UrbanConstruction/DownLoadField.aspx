<%@ Page Language="C#" MasterPageFile="~/WebSite.Master" AutoEventWireup="true" CodeBehind="DownLoadField.aspx.cs"
    Inherits="UrbanConstruction.DownLoadField" Title="下载中心" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    $(document).ready(function() {
        ...
        window.location.href = "DownLoadField.aspx?pageIndex=" + $("#mySelect").val() + "";
    });
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="middle">
        <!--S中间内容区域-->
        <p class="dw">
            您现在的位置： <a href="index.aspx">首页</a>&nbsp;&nbsp;<span>&gt;&gt;</span>&nbsp;&nbsp;<%=menuname %></p>
        <div class="List">
            <!--S列表内容-->
            <div class="ListL">
                <div class="LZCFG"><!--S下载中心-->
                	<h2><span>下载中心</span></h2>
                	<ul>
                	<%for (int i = 0; i < menulist.Count; i++)
                   { %>
                        <li>
                        <a href="DownLoadField.aspx?kind=<%=menulist[i].Kind.ToString() %>">
            ...
            <div class="ListR">
                <ul>
                    <%for (int i = 0; i < newsList.Count; i++)
                      {%>
                    <li><a href="DownLoadField.aspx?DID=<%=newsList[i].ID.ToString() %>" target="_blank">
                          <%if (newsList[i].FileName.Length > 32)
                          {%>
                             <%=newsList[i].FileName.Substring(0, 32)%>...
                          <%} %>
                          <%else
                            { %>
                             <%=newsList[i].FileName%>
                            ...
                            <%=newsList[i].AddTime.ToString("yyyy-MM-dd") %></span> </li>
                    <%} %>
                </ul>
                <p class="Page">
                    ...
                    <%else
                       ...
                       <%if (kindid != "")
                          {%>
                           <a href="DownLoadField.aspx?kind=<%=kindid %>&pageIndex=1">第一页</a>&nbsp;&nbsp;
                           <a href="DownLoadField.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex-1 %>">上一页</a>&nbsp;&nbsp;
                          <%} %>
                          <%else
                            { %>
                             <a href="DownLoadField.aspx?pageIndex=1">第一页</a>&nbsp;&nbsp;
                             <a href="DownLoadField.aspx?pageIndex=<%=pageIndex-1 %>">上一页</a>&nbsp;&nbsp;
                            <%} %>
                    <%} %>
                    ...
                    <%else
                        ...
                        <%if (kindid != "")
                          {%>
                            <a href="DownLoadField.aspx?kind=<%=kindid %>&pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                            <a href="DownLoadField.aspx?kind=<%=kindid %>&pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                          <%} %>
                          <%else
                            { %>
                              <a href="DownLoadField.aspx?pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                              <a href="DownLoadField.aspx?pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                          <%} %>
                    <%} %>
