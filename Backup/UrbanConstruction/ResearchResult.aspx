<%@ Page Language="C#" MasterPageFile="~/WebSite.Master" AutoEventWireup="true" CodeBehind="ResearchResult.aspx.cs" Inherits="UrbanConstruction.ResearchResult" Title="编研成果" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
<script src="js/jquery-1.9.1.js" type="text/javascript"></script>
<script type="text/javascript">
    $(document).ready(function() {
        $("#mySelect").change(function() {
        window.location.href = "ResearchResult.aspx?pageIndex=" + $("#mySelect").val() + "";
        });
    });
</script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
 <div class="middle"><!--S中间内容区域-->
    	<p class="dw">您现在的位置： <a href="index.aspx">首页</a>&nbsp;&nbsp;<span>&gt;&gt;</span>&nbsp;&nbsp;编研成果</p>
        ...
        <ul>
            ...
            <div class="ListR">
               ...
               {%>
               <%if (newsList[i].Type == videotype)
                 {%>
                <li>
                <a href="VideoDetail.aspx?FID=<%=newsList[i].ID.ToString()%>&token=<%=Session["CRSFToken"] %>" target="_blank">
                ...
                <span><%=newsList[i].ReleaseTime.ToString("yyyy-MM-dd") %></span>
               </li>
               <%} %>
               <%else{ %>
               <li>
                <a href="Books.aspx?FID=<%=newsList[i].ID.ToString()%>" target="_blank">
                  <%if (newsList[i].Title.Length > 32)
                          {%>
                             <%=newsList[i].Title.Substring(0,32)%>...
                          <%} %>
                ...
                <%else
                    { %>
                     <a href="ResearchResult.aspx?pageIndex=1">第一页</a>&nbsp;&nbsp;
                     <a href="ResearchResult.aspx?pageIndex=<%=pageIndex-1 %>">上一页</a>&nbsp;&nbsp;
                <%} %>
                ...
                <%else
                    { %>
                      <a href="ResearchResult.aspx?pageIndex=<%=pageIndex+1 %>">下一页</a>&nbsp;&nbsp;
                      <a href="ResearchResult.aspx?pageIndex=<%=pageCount %>">尾页</a>&nbsp;&nbsp;
                <%} %>
